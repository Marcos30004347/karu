__kernel void bsMV_kernel(
	...
	const unsigned long int y_block_heigth,
	const unsigned long int y_block_width,
  ...
  if(r < bs)
  {
    ...
    {
      unsigned long int target_block_col = col_ids[block];
			unsigned long int pos = target_block_col + t_idx;
      ...
      shared_x[t_idx] = x[stride + by*x_block_width + bx];
      barrier(CLK_LOCAL_MEM_FENCE);
