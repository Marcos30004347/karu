...
{
	compute::Buffer A_col_idx = compute::Buffer(A->bcsr_col_idx.data(), sizeof(u64)*A->bcsr_col_idx.size(), compute::Buffer::READ_WRITE, true);
	compute::Buffer A_row_ptr = compute::Buffer(A->bcsr_row_ptr.data(), sizeof(u64)*A->bcsr_row_ptr.size(), compute::Buffer::READ_WRITE, true);
	compute::Buffer A_buff		= compute::Buffer(A->bcsr_data.data(), 		sizeof(f32)*A->bcsr_data.size(), 		compute::Buffer::READ_WRITE, true);
	compute::Buffer x_buff 		= compute::Buffer(x->bcsr_data.data(), 		sizeof(f32)*x->bcsr_data.size(),	 	compute::Buffer::READ_WRITE, true);

	compute::Buffer y_buff 		= compute::Buffer(sizeof(f32)*y->bcsr_data.size(), compute::Buffer::WRITE_ONLY, compute::Buffer::MEM_GPU);

	A_col_idx.toComputeUnit();
	A_row_ptr.toComputeUnit();

	A_buff.toComputeUnit();
	x_buff.toComputeUnit();
	...
	bsMV_kernel->setKernelArgument(4, BUFFER_ARG_SIZE, A_col_idx.ref());
	bsMV_kernel->setKernelArgument(5, BUFFER_ARG_SIZE, A_row_ptr.ref());
	bsMV_kernel->setKernelArgument(6, BUFFER_ARG_SIZE, A_buff.ref());
	bsMV_kernel->setKernelArgument(7, BUFFER_ARG_SIZE, x_buff.ref());
	bsMV_kernel->setKernelArgument(8, BUFFER_ARG_SIZE, y_buff.ref());
	...
	f32* data = (f32*)y_buff.data();

	for(u64 i=0; i<x->bcsr_lines; i++)
	{
		std::cout << data[i] << " ";
	}
